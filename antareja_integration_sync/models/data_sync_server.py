...
class ExternalServerSync(models.Model):
    ...
    def jsonrpc_call(self, model, method, args, kw=None, db=None, uid=None, password=None):
        rec = self.ensure_one()
        if rec.external_mode == 'server_auth':
            return rec.application_server_auth_id.jsonrpc_call(model, method, args, kw=kw, db=db, uid=uid, password=password)

        if rec.external_mode == 'server_path':
            return rec.application_server_path_id.application_server_auth_id.jsonrpc_call(model, method, args, kw=kw, db=db, uid=uid, password=password)

        return super(ExternalServerSync, self).jsonrpc_call(model, method, args, kw=kw, db=db, uid=uid, password=password)

    # def get_external_data(self, model_name, domain=None, fields=None, offset=None, limit=None, count=False, object_id=None, context=None):
