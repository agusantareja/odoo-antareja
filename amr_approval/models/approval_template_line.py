...
class ApprovalTemplateLineMixin(models.AbstractModel):
    ...
    approve_action_type = fields.Selection([
        ...
        ('method', 'Method'),
        ('set_approve_state', 'Set State'),
    ], default='set_approve_state')
    ...
    reject_action_type = fields.Selection([
        ...
        ('server_action', 'Server Action'),
        ('method', 'Method'),
        ('set_reject_state', 'Set State'),
    ], default='set_reject_state')
    ...
    cancel_action_type = fields.Selection([
        ...
        ('server_action', 'Server Action'),
        ('method', 'Method'),
        ('set_canceled_state', 'Set Canceled'),
    ])
    ...
    def invoke_method(self, aprpoval_task_line, method_name, kwargs=None, raise_exceptions=False):
        atts_method_name = f"invoke_{method_name}"
        object_method_name = getattr(self, atts_method_name)
        if have_method(aprpoval_task_line, object_method_name):
            return safe_call_method(aprpoval_task_line, object_method_name, kwargs=kwargs)
        else:
            if raise_exceptions:
                raise UserError("object_method_name %s not found." % object_method_name)
            _logger.info("%s object_method_name %s not found.", aprpoval_task_line, object_method_name)
            return None
        ...
        def to_dict(records, result_dict):
            ...
            if records and isinstance(records, models.Model):
                if records._name == 'res.users':
                    ...
                    records_old = result_dict.get('group_ids') or self.env['res.group'].browse()
                    records_old |= records
