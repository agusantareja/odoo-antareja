...
class ApprovalUserTask(models.AbstractModel):
    _inherit = 'approval.user.task.mixin'
    ...
    def set_user_execution_id(self):
        self.ensure_one()
        super(ApprovalUserTask, self).set_user_execution_id()
        user_delegate_id = self.env.context.get('__user_delegate_id')
        if user_delegate_id:
            user_delegate = self.user_delegate_id.browse(user_delegate_id)
            self.write({
                'user_execution_id': user_delegate.proxy_id.id,
                'user_behalf_of_id': user_delegate.delegator_id.id
            })


class ApprovalTask(models.AbstractModel):
    ...
    def _approve_task(self):
        ...
        self_delegate = self.check_delegated_user()
        super(ApprovalTask, self_delegate)._approve_task()
    ...
    def _reject_task(self, **kwargs):
        ...
        self_delegate = self.check_delegated_user()
        super(ApprovalUserTask, self_delegate)._reject_task(**kwargs)
...
class ApprovalStrategyTaskMixin(models.AbstractModel):
    ...
    def prepare_dict_audit_trial(self):
        prepare_dict = super(ApprovalStrategyTaskMixin, self).prepare_dict_audit_trial()
        prepare_dict.update({
