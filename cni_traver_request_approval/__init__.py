...
def post_init_hook(cr, registry):
    ...
    records = env[model].search([('state', 'in', ['submit','cost_control','hc_approval'])])
    for rec in records:
        try:
    ...
    records = env['travel.approval'].search(
        [('approval_audit_log_id', '=', False),('travel_id', '>', 0),('state','=','approved')]
    )
