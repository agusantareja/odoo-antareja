...
# Bagian ini di gunakan untuk membantu share data dari system HR ke system external
...
class HrEmployee(models.Model):
    ...
    mobile_phone = fields.Char(related="mobile_phone")
    work_email = fields.Char(related="email")
    termination_date = fields.Date(related="resign_date")
    ...
    exp_date_contract = fields.Date(
        compute='_compute_exp_date_contract',
        store=True,
        help="Tanggal berakhir kontrak: diambil dari kontrak aktif terakhir, "
             "jika tidak ada maka dari kontrak closed terakhir"
    )
    ...
    def _compute_exp_date_contract(self):
        Contract = self.env['hr.contract']
        for employee in self:
            # 1. cari kontrak aktif terbaru
            active_contract = Contract.search([
                ('employee_id', '=', employee.id),
                ('state', '=', 'open'),
            ], order='date_start desc', limit=1)

            if active_contract and active_contract.date_end:
                employee.exp_date_contract = active_contract.date_end
                continue

            # 2. jika tidak ada kontrak aktif → ambil closed terakhir
            closed_contract = Contract.search([
                ('employee_id', '=', employee.id),
                ('state', '=', 'close'),
                ('date_end', '!=', False),
            ], order='date_end desc', limit=1)

            employee.exp_date_contract = closed_contract.date_end if closed_contract else False

    contract_ids = fields.One2many('hr.contract', 'employee_id', order="date_end desc")

    # ,hct.name as employee_category
    # left join hr_contract_type hct on hct.id = f.type_id
    # left join (select * from hr_contract where state in ('open','to_close') order by id desc) f on f.employee_id = a.id
    #         left join hr_contract_type hct on hct.id = f.type_id """ % host
    latest_contract_id = fields.Many2one(
        'hr.contract',
        compute='_compute_latest_contract',
        store=True,
        help="Menyimpan informasi kontrak terbaru"
    )
    employee_category = fields.Char(related='latest_contract_id.type_id.name', store=True)

    @api.depends('contract_ids', 'contract_ids.date_start', 'contract_ids.state')
    def _compute_latest_contract(self):
        for employee in self:
            # ambil kontrak terbaru (opsional: hanya active)
            contracts = employee.contract_ids.filtered(
                lambda c: c.state in ['open', 'to_close']
            )
            employee.latest_contract_id = contracts[:1] if contracts else False
    ...
    def _compute_gender(self):
        for rec in self:
            if rec.jenis_kelamin == 'laki-laki':
                rec.gender = 'male'
            elif rec.jenis_kelamin == 'perempuan':
                rec.gender = 'female'
            else:
                rec.gender = False

    # yang lebih susah untuk di deteksi perubahaanya
    # ,e.name as point_allocation
    # left join hr_point_allocation e on e.id = a.point_allocation_id
    point_allocation = fields.Char(related='point_allocation_id.name', store=True)
    # ,concat('%s/web/image?model=hr.employee&field=image_128&id=',a.id) as image_url
    image_url = fields.Char(compute='compute_image_url')

    def compute_image_url(self):
        web_base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        for rec in self:
            rec.image_url = "%s/web/image?model=hr.employee&field=image_128&id=%s" % (web_base_url, rec.id)

    # departement di replace dari hr_job
    # ,case
    ...
    classification_job = fields.Char('Classification Job', related='job_id.classification_job_id.name', store=True)
    #
    # ,k.name as bank_name
    # left join res_bank k on k.id = a.bank_id
    bank_name = fields.Char(related='bank_id.name', store=True)
    #
    # ,CASE
    ...
    spl_admin = fields.Boolean(string='SPL Admin', compute='_compute_spl_admin', store=True)

    @api.depends('spl_administrator_ids')
    def _compute_spl_admin(self):
        for employee in self:
            # ambil kontrak terbaru (opsional: hanya active)
            contracts = employee.contract_ids.filtered(
                lambda c: c.state in ['open', 'to_close']
    ...
    spl_administrator_ids = fields.One2many("cni.spl.administrator", 'employee_id', order="date_end desc")
